// Detail1 ~ Detail9까지 각각 user1 ~ user9의 실시간그래프 3개 그리고 있음
// 동일 코드이나 did(device ID)에 따라 받아오는 활력징후신호값이 다르므로 did가 변경될 시에 did 수정이 필요함
// user1의 활력징후신호(심박수, 호흡수, 체온) 그래프
...
const Detail = () => {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      // 실제 디바이스에서 user1의 device id가 1로 고정되어 있음, 만일 바뀔시에 바꿔주어야 함
      if (did === '1'){
